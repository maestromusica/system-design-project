...
trackbars = False
colormodes = np.array(['red','yellow','green','blue'])
displaymodes = np.array(['frame','blur','hsv','mask','res'])
...
def dumpConfiguration(params,filename):
    ...
    f.close()
    print('Color_configuration saved to file : {}'.format(filename+'.pkl'))
...
def display(colormode,mode,images):
...
def main():
    ...
    while True:
        ...
        # displaying images
        display(colorMode,displayMode,[frame,blur,hsv,mask,res])
