...
class wsFinder(object):
    ...
    def find(self,img):
        #flip the image horizontally as machine coords are the opposide direction
        flipped_img = cv2.flip(img,1)
        #undistort the image
        corrected_image = cv2.undistort(src=flipped_img, cameraMatrix=self.mtx, distCoeffs=self.dist, newCameraMatrix=self.optmtx)
        ...
        for k, m in masks.items():
            ...
            if len(contours) > 0:
                for box, rect in contours:
                    ...
                    boxes.append(Box(centroid, rect[1][1],rect[1][0],k,rect[2]))
